...
class TestPurchaseOrderViewSet:
    ...
    def test_list_own_orders(self, auth_client, user, org):
        PurchaseOrderFactory(organization=org, product=ProductFactory(organization=org))
        PurchaseOrderFactory(organization=org, product=ProductFactory(organization=org))
        resp = auth_client.get(reverse("purchase-order-list"))
        assert resp.status_code == status.HTTP_200_OK
        assert resp.data["count"] == 2

    def test_create_draft_order(self, auth_client, user, org):
    ...
    def test_confirm_purchase_order_creates_stock(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        po = PurchaseOrderFactory(organization=org, product=product, quantity=Decimal("100"))
    ...
    def test_cancel_draft_order(self, auth_client, user, org):
        po = PurchaseOrderFactory(organization=org, product=ProductFactory(organization=org))
        resp = auth_client.post(reverse("purchase-order-cancel", args=[po.pk]))
        assert resp.status_code == status.HTTP_200_OK
        assert resp.data["status"] == OrderStatus.CANCELLED

    def test_delete_only_draft(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        po = PurchaseOrderFactory(organization=org, product=product)
        po = confirm_purchase_order(po, "LOT-DEL")
        resp = auth_client.delete(reverse("purchase-order-detail", args=[po.pk]))
        assert resp.status_code == status.HTTP_400_BAD_REQUEST

    def test_filter_by_status(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        PurchaseOrderFactory(organization=org, product=product, status=OrderStatus.DRAFT)
        po2 = PurchaseOrderFactory(organization=org, product=product)
    ...
    def test_data_isolation(self, auth_client, user):
        ...
        PurchaseOrderFactory(organization=other_org, product=ProductFactory(organization=other_org))
        resp = auth_client.get(reverse("purchase-order-list"))
        assert resp.data["count"] == 0

    def test_cross_org_detail_returns_404(self, auth_client):
        other_org = OrganizationFactory()
        po = PurchaseOrderFactory(organization=other_org, product=ProductFactory(organization=other_org))
...
class TestSalesOrderViewSet:
    ...
    def test_list_own_orders(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        stock = StockFactory(organization=org, product=product, quantity=Decimal("200"))
        SalesOrderFactory(organization=org, product=product, stock=stock)
        resp = auth_client.get(reverse("sales-order-list"))
        assert resp.status_code == status.HTTP_200_OK
        assert resp.data["count"] == 1

    def test_create_sales_order(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        stock = StockFactory(organization=org, product=product, quantity=Decimal("200"))
    ...
    def test_confirm_sales_order_decrements_stock(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        stock = StockFactory(organization=org, product=product, quantity=Decimal("100"))
        so = SalesOrderFactory(
            organization=org, product=product, stock=stock, quantity=Decimal("30")
        )
    ...
    def test_confirm_fails_insufficient_stock(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        stock = StockFactory(organization=org, product=product, quantity=Decimal("5"))
        so = SalesOrderFactory(
            organization=org, product=product, stock=stock, quantity=Decimal("100")
        )
        resp = auth_client.post(reverse("sales-order-confirm", args=[so.pk]))
        assert resp.status_code == status.HTTP_400_BAD_REQUEST

    def test_cancel_confirmed_so_restores_stock(self, auth_client, user, org):
        product = ProductFactory(organization=org)
        stock = StockFactory(organization=org, product=product, quantity=Decimal("100"))
        so = SalesOrderFactory(
            organization=org, product=product, stock=stock, quantity=Decimal("30")
    ...
    def test_data_isolation(self, auth_client, user):
        ...
        stock = StockFactory(organization=other_org, product=product, quantity=Decimal("50"))
        SalesOrderFactory(organization=other_org, product=product, stock=stock)
        resp = auth_client.get(reverse("sales-order-list"))
        assert resp.data["count"] == 0
